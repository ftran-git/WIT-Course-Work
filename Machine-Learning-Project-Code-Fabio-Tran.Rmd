...
To only include the most statistically significant predictors in the Linear Regression model, I used the summary() function to compare coefficient p-values. The predictors: citric.acid, chlorides, and total.sulfur.dioxide had high p-values greater than 0.05 which does not prove statistical significance so I choose to ignore them. I also used the cor() function to compare correlation values to see which of the remaining predictors have the strongest correlation to the output variable quality. I choose the top 6 predictors with the strongest correlation which are ï..fixed.acidity, volatile.acidity, residual.sugar, density, pH, and alcohol.
...
for (i in 1:4898){
  training = data[-c(i), ]
  test = data[c(i), ]
  linear_regression = lm(quality ~ ï..fixed.acidity + volatile.acidity + residual.sugar + density, data = data)
  input = data.frame(test$ï..fixed.acidity, test$volatile.acidity, test$residual.sugar, test$density)
  colnames(input) = c("ï..fixed.acidity", "volatile.acidity", "residual.sugar", "density")
  predicted_value = predict(linear_regression, newdata = input)
  actual_value = test$quality
  MSE = (actual_value - predicted_value)^2
  list = c(list, MSE)
...
for (i in 1:4898){
  training = data[-c(i), ]
  test = data[c(i), ]
  linear_regression = lm(quality ~ ï..fixed.acidity + volatile.acidity + residual.sugar + density + pH, data = data)
  input = data.frame(test$ï..fixed.acidity, test$volatile.acidity, test$residual.sugar, test$density, test$pH)
  colnames(input) = c("ï..fixed.acidity", "volatile.acidity", "residual.sugar", "density", "pH")
  predicted_value = predict(linear_regression, newdata = input)
  actual_value = test$quality
  MSE = (actual_value - predicted_value)^2
  list = c(list, MSE)
...
for (i in 1:4898){
  training = data[-c(i), ]
  test = data[c(i), ]
  linear_regression = lm(quality ~ ï..fixed.acidity + volatile.acidity + residual.sugar + density + pH + alcohol, data = data)
  input = data.frame(test$ï..fixed.acidity, test$volatile.acidity, test$residual.sugar, test$density, test$pH, test$alcohol)
  colnames(input) = c("ï..fixed.acidity", "volatile.acidity", "residual.sugar", "density", "pH", "alcohol")
  predicted_value = predict(linear_regression, newdata = input)
  actual_value = test$quality
  MSE = (actual_value - predicted_value)^2
  list = c(list, MSE)
...
Created a Logistic Regression model using the same predictors from model 6 of the Linear Regression section. The observations are split in half to be used as training and test data respectively. The model is trained with the training data and is used to classify each test data observation. This yielded a test error rate of 0.2089796 which means about 20.89% of the test data observations are classified incorrectly.

```{r}
new_data_training = new_data[-c(1:2449), ]
new_data_test = new_data[c(2449:4898), ]

logistic_regression = glm(quality01 ~ ï..fixed.acidity + volatile.acidity + residual.sugar + density + pH + alcohol, data = new_data_training, family = binomial)
...
Used the KNN model to make a prediction for an observation with certain parameter values. The outcome is 1 which means the observation is classified as high quality.

```{r}
newTest.X = data.frame(ï..fixed.acidity = c(6.2), volatile.acidity = c(0.120), "residual.sugar" = c(5.40), "density" = c(0.9914), "pH" = c(3.25), "alcohol" = c(11.4))
...
Created a Decision Tree model to predict the variable "quality" that incorporates every predictor used in model 6 of the Linear Regression section.
